...
timeout = 120\n\
retries = 5\n\
default-timeout = 120" > $HOME/.config/pip/pip.conf

# Install pip and dependencies with retry mechanism
...
RUN for i in {1..5}; do \
        python -m pip install -r $HOME/computer_use_demo/requirements.txt && break || \
        echo "Retry $i/5..." && \
        sleep 5; \
    done
